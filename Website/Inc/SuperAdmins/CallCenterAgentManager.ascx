<%@ Control Language="VB" AutoEventWireup="false" CodeFile="CallCenterAgentManager.ascx.vb"
    Inherits="SBCSuperAdmin.CallCenterAgentManager" %>
<%@ Register Src="~/Inc/SuperAdmins/CallCenterAgentEdit.ascx" TagName="CallCenterAgentEdit"
    TagPrefix="uc" %>
...
<div class="row">
    <div class="col-lg-12">
        ...
        <asp:Button ID="btnViewLock" runat="server" Text="View Lock" ToolTip="View lock Agents" CssClass="btn btn-default" />
    </div>
</div>
<div class="mbxl"></div>
<div class="row">
        ...
        <asp:DataGrid ID="dgAgents" runat="server" AutoGenerateColumns="false" CssClass="table table-hover table-bordered">
            ...
            <Columns>
                <asp:TemplateColumn ItemStyle-HorizontalAlign="Center" ItemStyle-Width="20">
                    ...
                    <ItemTemplate>
                        <asp:CheckBox type="checkbox" ID="chkID" runat="server" value='<%# SBCBL.std.SafeString(Container.DataItem("CallCenterAgentID")) & "|" & SBCBL.std.SafeString(Container.DataItem("Login")) %>'
                            name='Chosen_<%#Container.DataItem("CallCenterAgentID") %>' />
                    </ItemTemplate>
                </asp:TemplateColumn>
                <asp:TemplateColumn HeaderText="Login (Name)" ItemStyle-HorizontalAlign="Left">
                    <ItemTemplate>
                        <asp:LinkButton ID="lbtEdit" runat="server" CommandArgument='<%# SBCBL.std.SafeString(Container.DataItem("CallCenterAgentID")) & "|" & SBCBL.std.SafeString(Container.DataItem("Login")) %>'
    ...
    <div class="col-lg-6" id="tdAgentEdit" runat="server">
        <uc:CallCenterAgentEdit ID="ucCCAgentEdit" runat="server" />
    </div>
</div>
<div class="mbxl"></div>
