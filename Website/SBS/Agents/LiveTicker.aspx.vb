...
#Region "Bind data"
        ...
        Private Sub bindTickets()
            ...
            If ddlSubAgent.SelectedValue.Equals(ALL) AndAlso Not ddlPlayers.SelectedValue.Equals(ALL) Then 'ALL - ""
                ucTicketBetsGridAgent.LoadTicketBets((New CTicketManager).GetOpenTicketsByAgent(UserSession.UserID, EDate, ddlPlayers.Value, SelectedTypeOfBet, Nothing, SafeDouble(txtThreshold.Text)), ddlContext.SelectedValue, True)
                Return
            End If
            If ddlSubAgent.SelectedValue.Equals(ALL) AndAlso ddlPlayers.SelectedValue.Equals(ALL) Then 'ALL - ALL
                ucTicketBetsGridAgent.LoadTicketBets((New CTicketManager).GetOpenTicketsByAllSubAgent(getListSubAgentID(UserSession.UserID), EDate, SelectedTypeOfBet, SafeDouble(txtThreshold.Text)), ddlContext.SelectedValue, True)
                Return
            End If
            If SelectedPlayer.Equals(ALL) Then
                oTicketBets = (New CTicketManager).GetOpenTicketsByAllSubAgent(getListSubAgentID(SelectedSubAgent), EDate, SelectedTypeOfBet, SafeDouble(txtThreshold.Text))
            Else
                oTicketBets = (New CTicketManager).GetOpenTicketsByAgent(SelectedSubAgent, EDate, ddlPlayers.Value, SelectedTypeOfBet, Nothing, SafeDouble(txtThreshold.Text))
            End If

            ucTicketBetsGridAgent.LoadTicketBets(oTicketBets, ddlContext.SelectedValue, True)
        End Sub
