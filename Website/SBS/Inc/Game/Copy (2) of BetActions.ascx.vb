    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If Not checkBetting() Then
                Return
            End If
            If (UserSession.SelectedBetType(Me.SelectedPlayerID) <> "PROP") AndAlso (BetTypeActive.Equals(_sBetTheBoard, StringComparison.CurrentCultureIgnoreCase) OrElse BetTypeActive.Equals(_sParlay, StringComparison.CurrentCultureIgnoreCase)) Then
            ...
            End If
            If (UserSession.SelectedBetType(Me.SelectedPlayerID) <> "PROP") AndAlso (BetTypeActive.Equals(_sBetTheBoard, StringComparison.CurrentCultureIgnoreCase)) Then
...
#End Region
    ...
    Private Function LoadGames(ByVal psGameType As String, ByVal poRepeater As Repeater, Optional ByVal oDTAdded As Data.DataTable = Nothing) As Boolean
        ...
        If oGameHalfTimeOff IsNot Nothing Then
            If Not UserSession.FirstHalfOff(psGameType) AndAlso oGame.OneH And Not oGameHalfTimeOff.FirstHOff Then
                oGameShows.Add("1H", Nothing)
            End If
            If Not UserSession.IsSecondhalfOff(psGameType) AndAlso oGame.TwoH And Not oGameHalfTimeOff.SecondHOff Then
        ...
        If UCase(BetTypeActive.Replace(_sBetTheBoard, _sStraight)) = "STRAIGHT" Then
            If Not UserSession.Cache.GetGameQuarterOff(SuperAgentId, psGameType, "1QOff") Then
                If oGame.Quarter1 Then
                    oGameShows.Add("1Q", Nothing)
                End If

            End If
            If Not UserSession.Cache.GetGameQuarterOff(SuperAgentId, psGameType, "2QOff") Then
                If oGame.Quarter2 Then
                    oGameShows.Add("2Q", Nothing)
                End If
            End If
            If Not UserSession.Cache.GetGameQuarterOff(SuperAgentId, psGameType, "3QOff") Then
                If oGame.Quarter3 Then
                    oGameShows.Add("3Q", Nothing)
                End If

            End If
            If Not UserSession.Cache.GetGameQuarterOff(SuperAgentId, psGameType, "4QOff") Then
        ...
        For Each sContext As String In oGameShows.Keys.ToArray()
            ...
            Dim lstGameOnOffCurrent As List(Of CGameTypeOnOff) = UserSession.Cache.GetGameTypeOnOff(SuperAgentId, psGameType, True)
            Dim lstGameOnOff1H As List(Of CGameTypeOnOff) = UserSession.Cache.GetGameTypeOnOff(SuperAgentId, psGameType, False)
            If lstGameOnOffCurrent IsNot Nothing AndAlso lstGameOnOffCurrent.Count > 0 Then


                Dim oGameOnOffCurrent As CGameTypeOnOff = lstGameOnOffCurrent(0)
                '' filter by line off time for agent
                If UCase(sContext) <> "2H" Then
                    If UCase(sContext) = "1H" AndAlso lstGameOnOff1H.Count > 0 Then
                        Dim oGameOnOff1H As CGameTypeOnOff = lstGameOnOff1H(0)
                        sWhere &= String.Format(" and isnull(MinuteBefore,0) > {0}", oGameOnOff1H.OffBefore)
                    Else
                        sWhere &= String.Format(" and isnull(MinuteBefore,0) > {0}", oGameOnOffCurrent.OffBefore)
                    End If
                End If
                If UCase(sContext) <> "2H" Then
                    If UCase(sContext) = "1H" AndAlso lstGameOnOff1H.Count > 0 Then
                        Dim oGameOnOff1H As CGameTypeOnOff = lstGameOnOff1H(0)
                        sWhere &= String.Format(" and isnull(MinuteBefore,0) < {0}", (oGameOnOff1H.DisplayBefore * 60).ToString())
                    Else
                        sWhere &= String.Format(" and isnull(MinuteBefore,0) < {0}", (oGameOnOffCurrent.DisplayBefore * 60).ToString())
                    End If
                End If
            ...
            If IsFootball(psGameType) Or IsBasketball(psGameType) Then
                ...
                If sContext = "1Q" Or sContext = "2Q" Or sContext = "3Q" Or sContext = "4Q" Then
                    ...
                    If oGameQuarterDisplay IsNot Nothing Then
                        nQTimeoff = oGameQuarterDisplay.GameTimeOffLine
                        nQTimeDisplay = oGameQuarterDisplay.GameTimeDisplay
                        If nQTimeoff <> 0 Then
...
#End Region
    ...
    Protected Sub rptBets_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs)
        ...
        If chkTeamTotal.Checked AndAlso oDT.Rows.Count > 0 Then
            If (Not BetTypeActive.Equals(_sBetTheBoard, StringComparison.CurrentCultureIgnoreCase) AndAlso Not BetTypeActive.Equals(_sStraight, StringComparison.CurrentCultureIgnoreCase)) OrElse SafeDouble(oDT.Rows(0)("AwayTeamTotalPoints")) = 0 OrElse (SafeInteger(oDT.Rows(0)("MinuteBefore")) / 60) > _nOFFTeamTotal Then
    ...
    Protected Sub rptGameLines_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs)
        If e.Item.ItemType = ListItemType.AlternatingItem OrElse e.Item.ItemType = ListItemType.Item Then
            ...
            End If
            trDescription.Attributes("class") = "offering_pair_" & IIf(SafeString(ViewState("OddEven")).Equals("odd", StringComparison.CurrentCultureIgnoreCase), "even", "odd")
            ...
            If IsSoccer(SafeString(oData("GameType"))) Then
                ...
                lblAwaySpread.Text = safeVegass(AHFormat(nAwaySpread)).Replace("++", "+") & SafeString(IIf(nMoneyLine > 0, "&nbsp;+" & nMoneyLine, "&nbsp;" & nMoneyLine))
                If nAwaySpread = 0 Then
                    lblAwaySpread.Text = "PK&nbsp;" & SafeString(IIf(nMoneyLine > 0, "+" & nMoneyLine, nMoneyLine))
                End If
                nMoneyLine = _oOddsRuleEngine.GetMoneyLine(sGameID, sGameType, "Away", sContext, "TotalPoints", _
                ...
                lblHomeSpread.Text = safeVegass(AHFormat(nHomeSpread)).Replace("++", "+") & SafeString(IIf(nMoneyLine > 0, "&nbsp;+" & nMoneyLine, "&nbsp;" & nMoneyLine))
            ...
            If chkItem Is Nothing Then ''for team total
                ...
                If txtItem.ID.Contains("Away") Then
                    ...
                    If txtItem.ID.Contains("txtMoneyAwayTeamTotalOver") Then
                        nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Away", poData("Context"), "TeamTotalPoints", _
                                                      SafeDouble(poData("AwayTeamTotalPointsOverMoney")), True, "away", 0, False)
                    ...
                    ElseIf txtItem.ID.Contains("txtMoneyAwayTeamTotalUnder") Then
                        ...
                        nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Home", poData("Context"), "TeamTotalPoints", _
                                                     SafeDouble(poData("AwayTeamTotalPointsUnderMoney")), False, "away", 0, False)
                ...
                Else
                    ...
                    If txtItem.ID.Contains("txtMoneyHomeTeamTotalOver") Then
                        ...
                        nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Away", poData("Context"), "TeamTotalPoints", _
                                                      SafeDouble(poData("HomeTeamTotalPointsOverMoney")), True, "home", 0, False)
                    ...
                    ElseIf txtItem.ID.Contains("txtMoneyHomeTeamTotalUnder") Then
                        nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Home", poData("Context"), "TeamTotalPoints", _
                                                     SafeDouble(poData("HomeTeamTotalPointsUnderMoney")), False, "home", 0, False)
...
PropGame:
        ...
        Select Case True 'UCase(SafeString(sHABet))
            Case txtItem.ID.Contains("AwaySpread") '"AWAY_SPREAD", "HOME_SPREAD"thuong
                ...
                If ddlBuyPoint IsNot Nothing AndAlso ddlBuyPoint.Visible AndAlso ddlBuyPoint.SelectedIndex > 0 Then
                    Dim arrBuypoin As Array = ddlBuyPoint.SelectedValue.Split("|")
                    nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Away", poData("Context"), "Spread", _
                                             SafeDouble(poData("AwaySpreadMoney")) + SafeDouble(arrBuypoin(1)), pbFavorite, IncreaseSpread, 0, ddlBuyPoint.Visible)
                    'ClientAlert(SafeDouble(poData("AwaySpreadMoney")) + SafeDouble(arrBuypoin(1)) & "fdfd" & nMoneyLine, True)

                    oTicketBet = getSpeadBet(SafeDouble(SafeString(arrBuypoin(0))) + SafeDouble(SafeString(poData("AwaySpread"))), nMoneyLine, sChoice)
                   
                Else
                    nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Away", poData("Context"), "Spread", _
                                             SafeDouble(poData("AwaySpreadMoney")), pbFavorite, IncreaseSpread, 0, ddlBuyPoint.Visible)
            ...
            Case txtItem.ID.Contains("HomeSpread")
                ...
                If ddlBuyPoint IsNot Nothing AndAlso ddlBuyPoint.Visible AndAlso ddlBuyPoint.SelectedIndex > 0 Then
                    ...
                    nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Home", poData("Context"), "Spread", _
                                            SafeDouble(poData("HomeSpreadMoney")) + SafeDouble(arrBuypoin(1)), pbFavorite, IncreaseSpread, 0, ddlBuyPoint.Visible)

                    oTicketBet = getSpeadBet(SafeDouble(SafeString(poData("HomeSpread"))) + SafeDouble(arrBuypoin(0)), nMoneyLine, sChoice)
                Else
                    nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Home", poData("Context"), "Spread", _
                                            SafeDouble(poData("HomeSpreadMoney")), pbFavorite, IncreaseSpread, 0, ddlBuyPoint.Visible)
            ...
            Case txtItem.ID.Contains("AwayTotal") '"OVER_POINT", "UNDER_POINT"
                If ddlBuyPoint IsNot Nothing AndAlso ddlBuyPoint.Visible AndAlso ddlBuyPoint.SelectedIndex > 0 Then
                    ...
                    nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Away", poData("Context"), "TotalPoints", _
                                                      SafeDouble(poData("TotalPointsOverMoney")) + SafeDouble(arrBuypoin(1)), True, "", 0, ddlBuyPoint.Visible)
                    sChoice = "OVER"
                ...
                Else
                    ...
                    nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Away", poData("Context"), "TotalPoints", _
                                                 SafeDouble(poData("TotalPointsOverMoney")), True, "", 0, ddlBuyPoint.Visible)
                    sChoice = "OVER"
            ...
            Case txtItem.ID.Contains("HomeTotal") '"OVER_POINT", "UNDER_POINT"
                If ddlBuyPoint IsNot Nothing AndAlso ddlBuyPoint.Visible AndAlso ddlBuyPoint.SelectedIndex > 0 Then
                    ...
                    nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Home", poData("Context"), "TotalPoints", _
                                                               SafeDouble(poData("TotalPointsUnderMoney")) + SafeDouble(arrBuypoin(1)), False, "", 0, ddlBuyPoint.Visible)
                    sChoice = "UNDER"
                    oTicketBet = getTotalPointsBet(SafeDouble(poData("TotalPoints")) + SafeDouble(arrBuypoin(0)) _
                                                    , nMoneyLine _
                                                    , sChoice)
                ...
                Else
                    Dim arrBuypoin As Array = ddlBuyPoint.SelectedValue.Split("|")
                    nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Home", poData("Context"), "TotalPoints", _
                                                               SafeDouble(poData("TotalPointsUnderMoney")), False, "", 0, ddlBuyPoint.Visible)
                    sChoice = "UNDER"
                    oTicketBet = getTotalPointsBet(SafeDouble(poData("TotalPoints")) _
                                                    , nMoneyLine _
                                                    , sChoice)
            ...
            Case txtItem.ID.Contains("MoneyAwayMLine") '"AWAY_LINE"
                ...
                nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Away", poData("Context"), "MoneyLine", _
                                                       SafeDouble(poData("AwayMoneyLine")), pbFavorite, "", 0, False)
            ...
            Case txtItem.ID.Contains("MoneyHomeMLine")
                ...
                nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Home", poData("Context"), "MoneyLine", _
                                                          SafeDouble(poData("HomeMoneyLine")), pbFavorite, "", 0, False)
                oTicketBet = getMoneyLineBet(nMoneyLine, sChoice)
            Case txtItem.ID.Contains("MoneyDraw") '"DRAW_LINE"
                ...
                nMoneyLine = _oOddsRuleEngine.GetMoneyLine(SafeString(poData("GameID")), poData("GameType"), "Away", poData("Context"), "Draw", _
                                                     SafeDouble(poData("DrawMoneyLine")), False, "", 0, False)
                oTicketBet.DrawMoneyLine = nMoneyLine
                If Not BetTypeActive.Replace(_sBetTheBoard, _sStraight).Equals(_sStraight, StringComparison.CurrentCultureIgnoreCase) Then
        ...
        Try
            ...
            If oTicketBet IsNot Nothing Then
                ...
                Dim sActionType = BetTypeActive.Replace(_sBetTheBoard, _sStraight)
                ...
                Select Case sChoice
                    ...
                    Case Else
                        sTeamSide = sChoice
                End Select
                If oTicketBet.IsCircled AndAlso UCase(sActionType) = "STRAIGHT" Then
        ...
        For i As Integer = 0 To rptMain.Items.Count - 1
            ...
            For j As Integer = 0 To rptBets.Items.Count - 1
                ...
                For k As Integer = 0 To rptGameLines.Items.Count - 1
                    ...
                    If Not String.IsNullOrEmpty(txtMoneyAwayTeamTotalOver.Text) Then
                    ...
                    End If
                    If Not String.IsNullOrEmpty(txtMoneyAwayTeamTotalUnder.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        ...
                        End If
                    End If
                    If Not String.IsNullOrEmpty(txtMoneyHomeTeamTotalOver.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                    ...
                    If Not String.IsNullOrEmpty(txtMoneyHomeTeamTotalUnder.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                    ...
                    If Not String.IsNullOrEmpty(txtMoneyAwaySpread.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        ...
                        End If
                    End If
                    If Not String.IsNullOrEmpty(txtMoneyAwayMLine.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        Dim nAwaySpread As Single = SafeSingle(oData("AwaySpread"))
                        Dim nHomeSpread As Single = SafeSingle(oData("HomeSpread"))
                        getTeamJuice(nAwaySpread, nHomeSpread, bAwaySpreadJuice, bHomeSpreadJuice, bAwayMoneyLineJuice, bHomeMoneyLineJuice)
                        ...
                        End If
                    End If
                    If txtMoneyAwayMLine2.Visible AndAlso Not String.IsNullOrEmpty(txtMoneyAwayMLine2.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        Dim nAwaySpread As Single = SafeSingle(oData("AwaySpread"))
                        Dim nHomeSpread As Single = SafeSingle(oData("HomeSpread"))
                        getTeamJuice(nAwaySpread, nHomeSpread, bAwaySpreadJuice, bHomeSpreadJuice, bAwayMoneyLineJuice, bHomeMoneyLineJuice)
                        BetGame(lblMoneyAwayMLine2.Text.Substring(2).Contains("+"), txtMoneyAwayMLine2, bAwayMoneyLineJuice, oData, ddlBuyPointHomeSpread, True)
                        If StraightError Then
                            Return False
                        End If
                    End If
                    If Not String.IsNullOrEmpty(txtMoneyAwayTotal.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        ...
                        End If
                    End If
                    If Not String.IsNullOrEmpty(txtMoneyHomeSpread.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        Dim nAwaySpread As Single = SafeSingle(oData("AwaySpread"))
                        Dim nHomeSpread As Single = SafeSingle(oData("HomeSpread"))
                        getTeamJuice(nAwaySpread, nHomeSpread, bAwaySpreadJuice, bHomeSpreadJuice, bAwayMoneyLineJuice, bHomeMoneyLineJuice)
                        ...
                        End If
                    End If
                    If Not String.IsNullOrEmpty(txtMoneyHomeMLine.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        Dim nAwaySpread As Single = SafeSingle(oData("AwaySpread"))
                        Dim nHomeSpread As Single = SafeSingle(oData("HomeSpread"))
                        getTeamJuice(nAwaySpread, nHomeSpread, bAwaySpreadJuice, bHomeSpreadJuice, bAwayMoneyLineJuice, bHomeMoneyLineJuice)
                        ...
                        End If
                    End If
                    If txtMoneyHomeMLine2.Visible AndAlso Not String.IsNullOrEmpty(txtMoneyHomeMLine2.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        Dim nAwaySpread As Single = SafeSingle(oData("AwaySpread"))
                        Dim nHomeSpread As Single = SafeSingle(oData("HomeSpread"))
                        getTeamJuice(nAwaySpread, nHomeSpread, bAwaySpreadJuice, bHomeSpreadJuice, bAwayMoneyLineJuice, bHomeMoneyLineJuice)
                        BetGame(lblMoneyHomeMLine2.Text.Substring(2).Contains("+"), txtMoneyHomeMLine2, bHomeMoneyLineJuice, oData, Nothing, True)
                        If StraightError Then
                            Return False
                        End If
                    End If
                    If Not String.IsNullOrEmpty(txtMoneyHomeTotal.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
                        ...
                        End If
                    End If
                    If Not String.IsNullOrEmpty(txtMoneyDraw.Text) Then
                        Dim oData = oGameLineManager.GetGameLinesByID(sGameLineID)
    ...
    Private Function GetTPointLock(ByVal poRow As DataRowView) As Boolean
        ...
        Select Case UCase(SafeString(poRow("Context")))
            Case "CURRENT"
                If olstGameTypeOnOffCurrent.Count > 0 Then
                    Return SafeString(poRow("IsGameTPointLocked")) = "Y" Or (60 * CType(olstGameTypeOnOffCurrent(0), CGameTypeOnOff).GameTotalPointsDisplay) < SafeInteger(poRow("MinuteBefore"))
                End If
                Return SafeString(poRow("IsGameTPointLocked")) = "Y"
            Case "1H"
                If olstGameTypeOnOff1H.Count > 0 Then
                    Return SafeString(poRow("IsFirstHalfTPointLocked")) = "Y" Or (60 * CType(olstGameTypeOnOff1H(0), CGameTypeOnOff).GameTotalPointsDisplay) < SafeInteger(poRow("MinuteBefore"))
                End If
            Case "2H"
                Return SafeString(poRow("IsSecondHalfTPointLocked")) = "Y"
            Case "1Q", "2Q", "3Q", "4Q"
                Return (60 * CType(olstGameTypeOnOffCurrent(0), CGameTypeOnOff).GameTotalPointsDisplay) < SafeInteger(poRow("MinuteBefore"))
        End Select
        Return False
    End Function
    ...
    Private Function GetMLLock(ByVal poRow As DataRowView) As Boolean
        ...
        Dim oSysManager As New CSysSettingManager()
        Dim oCachemanager As New CCacheManager()
        Dim oSysGame As CSysSetting = oCachemanager.GetAllSysSettings(_sGameType).Find(Function(x) x.Key = sGameType AndAlso x.SubCategory <> "")
        Dim oSysSport As CSysSetting = oCachemanager.GetAllSysSettings(_sGameType).Find(Function(x) x.SysSettingID = oSysGame.SubCategory)
        Dim sSportType As String = oSysSport.Key
    ...
    Public Function ShowCheckBox() As Boolean
        ...
        BetTypeActive.Equals("bettheboard", StringComparison.CurrentCultureIgnoreCase) OrElse LCase(BetTypeActive).Contains("if ") Then
            Return False
        Else
            Return True
        End If
    ...
    Public Function getHeader() As String
        If SafeBoolean(Session("BetTypeProp")) AndAlso BetTypeActive.Equals(_sBetTheBoard, StringComparison.CurrentCultureIgnoreCase) Then
            Return "Please enter amounts for one or more wagers in the charts below - then press"
        ElseIf BetTypeActive.Equals(_sReverse) Then
            Return "Please select two items from the chart below - then press "
        ElseIf BetTypeActive.Equals(_sBetTheBoard, StringComparison.CurrentCultureIgnoreCase) Then
            Return "Please enter amounts for one or more games in chart below - then press"
        Else
            Return "Please select items from the chart below - then press"
    ...
    Public Sub ParseBuyPointOptions(ByVal pnJuice As Double, ByVal pnBetPoint As Double, ByVal psBetType As String, ByVal psContext As String, ByVal psGameType As String, ByVal ddlBuyPoint As CDropDownList, ByVal lblOdd As Label, Optional ByVal bTotalOver As Boolean = False)
        ...
        If chkBuyPoint.Checked AndAlso (BetTypeActive.Equals(_sBetTheBoard, StringComparison.CurrentCultureIgnoreCase) OrElse BetTypeActive.Equals(_sParlay, StringComparison.CurrentCultureIgnoreCase)) Then
            ...
            If bBuyPoint Then
                ''Single Parlay
                Dim oDicItem As New DictionaryEntry(IIf(SafeString(pnJuice).Contains("-"), SafeString(pnJuice), "+" + SafeString(pnJuice)) & "   " & SafeString(pnBetPoint), "")
                ...
                If IsFootball(psGameType) Then
                    ...
                    If pnJuice = 3 OrElse pnJuice = -3 OrElse pnJuice = 2.5 OrElse pnJuice = -3.5 Then
                        oDicItem = New DictionaryEntry(IIf(SafeString(pnJuice + 0.5).Contains("-"), SafeString(pnJuice + 0.5), "+" + SafeString(pnJuice + 0.5)) & " (" & SafeString(pnBetPoint - 20) & ")", "0.5|-20")
                        olstDic.Add(oDicItem)
                    End If
                ...
                If olstDic.Count = 1 Then
                    ...
                    For nPoint As Double = 0.5 To 1.5 Step 0.5
                        ...
                        If UCase(psBetType) = "TOTALPOINTS" AndAlso bTotalOver Then
                            nAddpoint = -nPoint
                        End If
                        oDicItem = New DictionaryEntry(IIf(SafeString(pnJuice + nAddpoint).Contains("-"), SafeString(pnJuice + nAddpoint), "+" + SafeString(pnJuice + nAddpoint)) & "   " & SafeString(pnBetPoint + (-nPoint * 30)), _
                                                       SafeString(nAddpoint) & "|" & SafeString(-nPoint * 30))
                        olstDic.Add(oDicItem)
                    Next
                End If
            End If
            ' ClientAlert(olstDic.Count & "", True)
