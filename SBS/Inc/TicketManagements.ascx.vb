...
Partial Class SBS_Inc_TicketManagements
    ...
    Private Function GetSelectedAgent() As String
        'If UserSession.UserType = EUserType.Agent Then
        If (ddlAgents.SelectedValue = "" OrElse String.Equals(ddlAgents.SelectedValue, All, StringComparison.OrdinalIgnoreCase)) Then
...
#Region "Bind Data"
    ...
    Private Sub BindPlayers()
        Dim oPlayer As New CPlayerManager()
        Dim oDT As DataTable = oPlayer.GetPlayers(GetSelectedAgent(), Nothing)
...
#End Region
    ...
    Private Sub SearchTickets()
        ...
        Dim oDtTickets As DataTable = New DataTable()

        If UserSession.UserType = EUserType.SuperAdmin Then

            ' search ticket from all agents
            If GetSelectedAgent().Equals(UserSession.UserID) Then
                oDtTickets = oTicketManager.SearchTicketByAllSubAgent(getListAgentIDBySelectedAgent(GetSelectedAgent()), SafeInteger(txtTicket.Text))
            Else
                oDtTickets = oTicketManager.SearchTicketByAgent(GetSelectedAgent(), SafeInteger(txtTicket.Text), ddlPlayer.SelectedValue)
            End If

            '' search deleted ticket
            'Dim oDTDeleted = oTicketManager.GetDeletedTickets()
            ' '' get records of this month
            'Dim oDate As DateTime = GetLastMondayOfDate(GetEasternDate())
            ...
            '    Dim oRow As DataRow = oDtTickets.NewRow()
            ...
            '    oDtTickets.Rows.Add(oRow)
            'Next

            oDtTickets.DefaultView.Sort = "TicketNumber , SubTicketNumber ,  TransactionDate DESC ,  TicketID, TicketType"
            oDtTickets = oDtTickets.DefaultView.ToTable()
        Else
            oDtTickets = oTicketManager.SearchTicketByAgent(GetSelectedAgent(), SafeInteger(txtTicket.Text), ddlPlayer.SelectedValue)
        End If

        grdTickets.DataSource = oDtTickets
        grdTickets.DataBind()
        ...
        If grdTickets.Items.Count > 1 Then
            formatDisplayGridWithRowSpan(oDtTickets)
        End If
    ...
    Protected Sub btnReset_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReset.Click
        ...
        ddlPlayer.Items.Add(New ListItem(All, ""))
    End Sub
    ...
    Private Function loadAgents() As DataTable
        ...
        Dim dtParents As DataTable = GetDtAgents(UserSession.UserID)

        If dtParents Is Nothing OrElse dtParents.Rows.Count = 0 Then
            GoTo ReturnAgent
        End If
...
ReturnAgent:
    ...
    Private Function GetDtAgents(ByVal agentId As String) As DataTable
        Dim oAgentManager As New CAgentManager()
        Dim dtAgents As DataTable = New DataTable()
        If UserSession.UserType = EUserType.Agent Then
            dtAgents = oAgentManager.GetAllAgentsByAgent(agentId, Nothing)
        Else
            dtAgents = oAgentManager.GetAllAgentsBySuperAdminID(agentId, Nothing, pbIsSuperAmdin:=agentId = UserSession.UserID)
        End If
    ...
    Private Function getListAgentIDBySelectedAgent(ByVal psAgentID As String) As List(Of String)
        Dim olstAgent As List(Of String) = New List(Of String)
        Dim odtSubAgent As DataTable = GetDtAgents(psAgentID)
        For Each odrAgent As DataRow In odtSubAgent.Rows
            olstAgent.Add(SafeString(odrAgent("AgentID")))
        Next
        olstAgent.Add(psAgentID)
        Return olstAgent
    End Function
    ...
    Private Sub newDeleteTickets()
        ...
        For Each ticketItem As DataGridItem In grdTickets.Items
            If ticketItem.FindControl("chkDelete") IsNot Nothing AndAlso CType(ticketItem.FindControl("chkDelete"), CheckBox).Checked Then
                ...
                Dim oTicketManager As New SBCBL.Managers.CTicketManager()
                Dim HFPlayerID As HiddenField = CType(ticketItem.FindControl("hFPlayerID"), HiddenField)
                bDeleteSuccess = oTicketManager.NewDeleteTicket(CType(ticketItem.FindControl("chkDelete"), CheckBox).ValidationGroup, HFPlayerID.Value)
            End If
        Next
        If bDeleteSuccess Then
            ClientAlert("Delete Successfull", True)
    ...
    Private Sub unDeleteTickets()
        Dim bUnDeleteSuccess As Boolean = False
        For Each ticketItem As DataGridItem In grdTickets.Items
            If ticketItem.FindControl("chkUnDelete") IsNot Nothing AndAlso CType(ticketItem.FindControl("chkUnDelete"), CheckBox).Checked Then

                Dim oTicketManager As New SBCBL.Managers.CTicketManager()
                Dim HFPlayerID As HiddenField = CType(ticketItem.FindControl("hFPlayerID"), HiddenField)
                bUnDeleteSuccess = oTicketManager.UnDeleteTicket(CType(ticketItem.FindControl("chkUnDelete"), CheckBox).ValidationGroup, HFPlayerID.Value)
            End If
        Next
        If bUnDeleteSuccess Then
            ClientAlert("UnDelete Successfull", True)
        Else
            ClientAlert("UnDelete Fail", True)
        End If
        SearchTickets()
    End Sub
