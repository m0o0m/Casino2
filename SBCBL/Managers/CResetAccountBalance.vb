...
Public Class CResetAccountBalance
    ...
    Public Sub ResetAccountBalance(ByVal odrPlayer As DataRow, ByVal odrAgent As DataRow)

        '' Update Player's OriginalAmount first
        UpdatePlayerOriginalAmount(SafeString(odrPlayer("PLayerID")))
...
#Region "Reset Player's Amount"
    ...
    Public Function UpdatePlayerOriginalAmount(ByVal psPlayerID As String) As Boolean

        Dim bSuccess As Boolean = True

        Dim oUpdate As New CSQLUpdateStringBuilder("Players", "WHERE Players.OriginalAmount <> 0 and PlayerID=" & SQLString(psPlayerID))
        Dim odbSQL As New CSQLDBUtils(SBC_CONNECTION_STRING, "")
...
#End Region
    ...
    Private Sub AddTransaction(ByVal pdrInfo As DataRow, ByVal pbPlayer As Boolean)
        ...
        Try
            ...
            Dim oEasternMon As Date = GetEasternDate()

            '' Get P/L amount
            If pbPlayer Then
                nAmountOwed = oTicketManager.GetPLAmount(SafeString(pdrInfo("PlayerID")), GetMondayOfCurrentWeek, oEasternMon, pbPlayer)
